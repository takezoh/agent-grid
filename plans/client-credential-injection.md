...
- **影響範囲**: `host/credproxy/` (browser-push を受領する Source 追加)、`clients/ui/` (secret store + 配布 UI + outbox)、host channel に frame 追加 (3 種)、ADR (credential source model / browser secret storage / Tier 1 vs Tier 2 / agent ごとの注入経路)
