...
- **影響範囲**: 新規 `clients/windows-shell/` (C#)、新規 `clients/workspace/` (Electron/TS)、`clients/ui` の hosted モード
...
SPA の配信元は現行構成のまま使う: SPA はゲートウェイ (`cmd/server`) ではなく**別バイナリ `cmd/uihost` (バイナリ名 `web`) が go:embed で配信**し、`/api` と `/ws` をゲートウェイへリバースプロキシしている (CSP と WS origin check はこの origin で成立)。よって Workspace は `http://127.0.0.1:<web-port>/?hosted=1&session=<id>` を BrowserWindow にロードする。`file://` ロードは origin check を壊すため採らない。Shell のデーモン監督対象に `cmd/uihost` プロセスも含める (server と同じ spawn/adopt、WSL 内)。
...
| DaemonSupervisor 統合 | fake server バイナリ (即応答する小さな Go プログラム、`src/server/api/testsupport/fakeagents` の流儀を踏襲) に対する spawn/adopt/restart。`wsl.exe` 境界は Runner インターフェースで抽象化し fake 差替 |
...
| hosted モード SPA | 既存 `clients/ui` の unit + Playwright に hosted フラグの分岐ケースを追加 (`e2e/support/fake-backend.ts` 再利用) |
...
2. ~~ゲートウェイが SPA を静的配信しているか~~ → 確認済み: SPA は `cmd/uihost` (embed + reverse proxy、バイナリ名 `web`) が配信。Workspace はその origin をロードする (§4.3)。残課題は hosted フラグの受け渡し形式 (query vs preload) のみ。
