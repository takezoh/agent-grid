...
- **Gateway**: 公開網に置く新規 binary。**relay / tunnel / authorizer の 3 責務のみ**を担い、ドメイン (session / frame / driver / agent / capability / ACL 中身 / host display name) を **持たないし解釈しない**。具体的責務:
  ...
  - **Relay**: 4 層をまとめて担う。**(i) WebRTC signaling** (SDP/ICE candidate を opaque に中継) / **(ii) STUN server** (RFC 5389/8489、browser/host の server-reflexive アドレス探索に応答。NAT 越え用の軽量プロトコルで relay ではないが ICE 候補収集に必要) / **(iii) TURN server** (RFC 5766/8656、NAT 越え不能時に DTLS/SCTP packet を relay。short-lived credential は authorizer が発行) / **(iv) opaque application relay** (WebRTC が DataChannel まで含めて確立できなかった host channel について、Noise ciphertext を素通し relay)
  - **Authorizer**: user identity (passkey 等) の認証と、user 署名済 op の opaque な配送、および **TURN short-lived credential** の発行 (STUN は auth 不要のため対象外)
...
6. **Gateway はドメイン情報を一切持たない** (relay/tunnel/authorizer の 3 責務のみ、relay は signaling/STUN/TURN/opaque の 4 層を内包)。host display name や capability snapshot は browser localStorage か host channel から取る。これにより gateway 侵害時の info leak が pubkey fingerprint と relay traffic pattern (TURN + opaque 双方の byte counter、STUN は stateful 状態を持たないため leak 対象にもならない) のみに縮小される。
...
- credential / secret の cross-host 同期。host ごと独立 (credential の出所を browser 側に置く follow-up は §19 / [client-credential-injection.md](./client-credential-injection.md))。
- web client native app (PWA / Tauri)。ブラウザ前提を維持 (client 機自身での hostexec は既存 host モデル再利用で実現する follow-up を §19 / [client-as-host.md](./client-as-host.md) に切り出した、本 plan 内で native 化はしない)。
...
Gateway は **relay / tunnel / authorizer の 3 責務のみ**、ドメイン情報を持たない (§0)。**data plane は browser ↔ host 直結** (LAN 直結が最優先、WebRTC P2P (host/srflx) が次、WebRTC + TURN relay、最後に application-layer の gateway opaque relay)。STUN は **relay ではなく NAT mapping 探索** に使われ、P2P / TURN tier に共通して効く。TURN と opaque relay は別物 — TURN は WebRTC layer で DTLS packet を relay、opaque relay は WebRTC を捨てて application Noise frame を relay する。
...
```
                     ...
                     │  - WebRTC signaling        │   ← relay (i)
                     │  - STUN server :3478       │   ← relay (ii)
                     │    (RFC 5389/8489)         │
                     │  - TURN server :3478/5349  │   ← relay (iii)
                     │    (RFC 5766/8656)         │
                     │  - opaque application      │   ← relay (iv)
                     │    relay fallback           │
                     │  - WebAuthn/passkey login  │   ← authorizer
                     │  - user-signed op forward  │   ← authorizer
                     │  - TURN short-lived cred   │   ← authorizer
                     │  - audit (pubkey + byte    │
                     │    counter のみ、TURN /     │
                     │    opaque 別系統で記録、    │
                     │    STUN は stateless)       │
                     ...
                     │   frame meta, ACL 中身      │
                     │                            │
                     │  WebRTC stack 本体は持たない│
                     │  (client side は browser /  │
                     │   pion-webrtc に任せる)     │
                     └──┬──────┬────────┬─────────┘
   signaling /          │      │        │      / signaling
   heartbeat (gRPC)     │      │        │
...
1. **LAN 直結** (mDNS 発見 + pubkey TOFU、gateway 完全不要)
2. **WebRTC P2P (host candidate / server-reflexive)** (gateway は signaling + STUN endpoint のみ提供。WebRTC stack が STUN binding を打って srflx を獲得 → ICE で P2P 確立 → 確立後の data は gateway を経由しない)
3. **WebRTC + TURN relay** (NAT 越え不能時のみ ICE が TURN relay candidate (gateway が short-lived credential を発行) を選ぶ。gateway は DTLS/SCTP の ciphertext packet を relay、WebRTC DataChannel 自体は維持)
4. **Gateway opaque application relay** (上記いずれも不能時の最終 fallback。WebRTC を捨てて Noise ciphertext を application stream として素通し relay。gateway は session の存在/件数を知らない)

STUN は単独 tier ではなく **2 (P2P 成立) を可能にするためのプロトコル**。Browser/host の WebRTC stack が ICE candidate gathering 中に gateway STUN endpoint を打って srflx を得る — gateway は応答するだけで stateful relay は不要 (per-binding 数 byte の UDP exchange のみ)。

3 と 4 は別 fallback tier。3 は WebRTC layer の packet relay (TURN allocation + DTLS)、4 は application layer の frame relay (gateway-controlled gRPC stream + 上位 Noise)。3 で済めば WebRTC stack をそのまま使い続けられるため state machine / 多重化のコードが共通化できる。

すべての経路で上位に **同じ Noise セッションを再利用**するため、経路切替はアプリ層から透過。経路は **host channel 単位**で選択され、host ごとに違う経路を同時に使う (例: H1 は LAN 直結、H2 は WebRTC P2P (srflx)、H3 は WebRTC+TURN、H4 は opaque relay fallback)。
...
- **[Relay] STUN server (RFC 5389/8489)**: browser/host の WebRTC stack が NAT mapping を探索するための応答 endpoint。`stun:gw.example.com:3478` を SignalingHints の `ice_servers` に乗せて配布。**auth 不要 / stateful 状態を持たない / per-binding 数 byte の UDP exchange のみ**。Public STUN (Google 等) を使う設計も可能だが、self-host gateway 環境では gateway 同居が外部依存を減らす。実装は TURN server と同一 process (pion/turn 系 server は STUN も同時に喋る)。
- **[Relay] TURN server (RFC 5766/8656)**: WebRTC ICE が host candidate / server-reflexive で NAT 越えに失敗した時の relay candidate を提供。`turn:` / `turns:` URI を SignalingHints の `ice_servers` に乗せて browser/host へ配布、authorizer が **short-lived credential** (HMAC-SHA1 username + ephemeral password、TTL は assertion token 同等以下) を発行する。relay する packet は **DTLS/SCTP ciphertext** のままで gateway は decrypt できない (SDP も signaling として gateway を素通っているだけで鍵は持たない)。実装は **(a) gateway-bundled pion/turn (STUN/TURN 一体)** / **(b) 外部 coturn instance を gateway authorizer が credential 払い出して指す** の 2 系統 (Q20)。WebRTC stack を維持するため上位 application は opaque application relay (下記) と区別しなくてよい。

  **WebRTC stack との分担 (再掲)**: gateway は STUN/TURN の **server 実装** を提供するのみ。client 側 (browser native / pion-webrtc on host) は ICE / DTLS / SCTP / DataChannel をすべて自前の stack で賄う。Gateway 側に WebRTC stack を入れない (ドメインを持たない原則とも整合 — gateway が DataChannel の semantics を見たら no-domain 違反になる)。
- **[Relay] Opaque application relay fallback**: TURN ですら ICE が確立しない場合 (UDP 完全遮断 + TURN/TCP/TLS も到達不能) の **最終 fallback**。WebRTC を諦め、host ↔ gateway control tunnel 内に新規 application stream を開き、browser ↔ gateway control WSS と pipe する。**gateway は session の存在も件数も認識しない**、byte counter のみ。Noise ciphertext を素通すだけで、SDP も DataChannel も介在しない。TURN (上記) との違い: TURN は WebRTC layer の packet relay、これは application layer の frame relay。
...
- **[Authorizer] TURN credential issuer**: 認証済 user に対して TURN long-term credential (RFC 8489) を short-lived に発行 (STUN は auth 不要のため対象外)。TTL は assertion token と同等以下、credential 単体では session metadata に到達できない (TURN allocation の宛先 / 件数も relay 対象 host pubkey とは紐付けない設計)。
...
- **[Tunnel] Audit log (pubkey + metadata only)**: 接続元 client pubkey、対象 host pubkey fingerprint、connection open/close、relay bytes 集計 (**TURN allocation 単位 / opaque application relay 単位を別系統で集計**)。**display name / session id / capability は記録しない** (gateway は元から知らない)。
- **[Tunnel] Health**: host heartbeat 集計、metrics export (control plane / relay / TURN allocation 観点のみ、session 単位 metric は host 側で expose、§6.5)。
...
gateway/
  ...
  authz/
    ...
    turn_cred.go     # TURN short-lived credential 発行 (RFC 8489 long-term auth, HMAC-SHA1 ephemeral password)
  stunturn/
    server.go        # STUN/TURN server を 1 process でホスト (RFC 5389/8489 + 5766/8656)。pion/turn embedding (案 a) か外部 coturn proxy (案 b、本 dir は credential 払い出しのみ) — Q20 で決定
    allocation.go    # TURN allocation lifecycle + per-credential rate limit + byte counter (STUN は stateless で対象外)
    bandwidth.go     # TURN session 単位の bandwidth quota
  relay_fallback/
    opaque.go        # TURN すら確立しない host channel の application 暗号文 stream を素通し relay
    bandwidth.go     # per-(client, host) rate limit + byte counter (TURN とは別系統で集計)
  audit/
    log.go           # connection metadata only (pubkey fp + TURN bytes + opaque relay bytes、session id / display name / capability は持たない)
  metrics/
    prometheus.go    # /metrics (TURN allocation / opaque relay を別 metric 名で expose)
cmd/gateway/
  main.go            # config load, edge bind, TURN bind, signal handling
```

#### 4.1.3 状態の永続化

- **必要**: host registry (**pubkey fingerprint + signaling hints のみ**、display name は持たない)、client identities (passkey credential)、pairing tokens、user 署名済 ACL op の opaque blob 履歴 (replay 配布用)、audit log (pubkey fp + TURN/opaque relay 別系統 byte counter)、TURN shared secret (credential 発行用、TURN server bundling case のみ)。
- **不要**: session state (host が持つ)、pty buffer (E2E のため不可)、host display name (browser localStorage)、capability snapshot (host channel 経由)、TURN allocation 履歴の長期保存 (allocation 終了で消す、audit には集計値のみ)。
...
| **Data plane (P2P)** | Browser ↔ Host (host/srflx) | WebRTC DataChannel (DTLS-SCTP) | DTLS + 上位 Noise | 1 PeerConnection / host、session ごと DataChannel (or 1 channel + 上位 mux) |
| **Data plane (TURN)** | Browser ↔ Gateway-TURN ↔ Host | WebRTC DataChannel (gateway STUN で srflx 取得 → TURN relay candidate; UDP / TCP / TURNS) | DTLS + 上位 Noise (gateway は DTLS ciphertext を packet relay) | WebRTC stack を維持 (P2P と同じ DataChannel 多重化) |
| **Data plane (opaque)** | Browser ↔ Gateway ↔ Host | application-layer stream を opaque relay | TLS + 上位 Noise (gateway は素通し) | 1 stream / host channel、WebRTC を捨てる場合のみ |
...
src/
  ...
  gateway/                              # NEW layer (relay / tunnel / authorizer only, no domain)
    edge/ registry/ authz/ stunturn/ relay_fallback/ audit/ metrics/
                                        # registry は pubkey fp + signaling hints (ICE/LAN/TURN URI) のみ
                                        # authz は user signed op の opaque forward + token issuance + TURN cred 発行
                                        # stunturn は STUN/TURN server (pion/turn) を 1 process で同居 (Q20 案 a)
  platform/
...
| Q20 | STUN/TURN provider: (a) gateway-bundled pion/turn / (b) 外部 coturn instance を gateway authorizer が指す / (c) Public STUN (Google) + 外部 TURN | 運用コスト / NAT 越え成功率 / 外部依存 | MVP: **(a) gateway-bundled (pion/turn 1 process で STUN + TURN を `:3478` + `:5349` に bind)**。Self-host 一体感と外部依存ゼロを優先。帯域コスト懸念が出たら (b) で外部 coturn へ寄せる (gateway は credential 発行のみ)。Public STUN は self-host gateway の主旨と合わないため不採用 (dev 環境では fallback として SignalingHints に複数 URI を載せる選択肢を残す) |
| Q21 | WebRTC client stack の選定 (host 側) | 実装複雑度・メモリ | **pion/webrtc (純 Go)** を一次案。CGO 回避 + Noise/gRPC ecosystem と整合。Browser 側は native WebRTC API (ブラウザ実装、選定不要)。Gateway 側には WebRTC stack を入れない (no-domain 原則と整合、STUN/TURN server 実装のみで足りる) |
| Q22 | STUN binding と P2P 確立に失敗してから TURN/relay へ降格するまでの timeout | UX (起動感) vs ネットワーク堅牢性 | ICE gathering 5s → TURN candidate 試行 5s → opaque relay へ落とす、を一次案。`signaling_timeout` reason code で UI に通知 (ADR-0046 系の reason code 規約) |
...
| WebRTC NAT 越え失敗時の TURN 帯域コスト | 運用費 | gateway 同居 pion/turn (Q20 案 a) で開始、帯域が問題化したら外部 coturn (案 b) へ寄せる。TURN ですら不能な host channel のみ opaque relay へ降格、それぞれの byte counter を audit で分けて運用 |
| STUN/TURN server を gateway に同居させたことで attack surface が広がる | gateway DoS / 既知 CVE | pion/turn を選択することで CGO・C 依存ゼロ。port 別 listen + rate limit + credential TTL で限定。外部 coturn 化への移行余地 (Q20 案 b) を残す |
...
3. **Gateway は relay / tunnel / authorizer の 3 責務のみ、ドメイン情報を持たない / 解釈しない** ([[feedback-gateway-no-domain]])。session / frame / driver / agent / capability / ACL 中身 / host display name は **gateway に存在しない**。Gateway が持つのは pubkey fingerprint / signaling hints / online 状態 / relay byte counter / user identity credential / TURN short-lived credential のみ。**relay は signaling / STUN / TURN / opaque application relay の 4 層を内包** し、それぞれ別 fallback tier として独立に動く。
4. **Data plane は browser ↔ host 直結**: LAN 直結 → WebRTC P2P (srflx) → WebRTC + TURN relay → opaque application relay の優先順位で動的選択。STUN は relay tier ではなく WebRTC P2P 成立用の NAT 探索プロトコル。WebRTC client stack (browser native / pion-webrtc on host) が ICE/DTLS/SCTP/DataChannel を **自前で賄う**ので gateway 側に WebRTC stack を入れる必要はない (gateway は STUN/TURN server endpoint と opaque relay endpoint を提供するだけ)。Noise は transport の上に被せ、経路切替で session を切らない。
...
## 19. 関連 follow-up plan

multi-host-gateway の wire shape (host channel + Noise + LAN direct + pubkey identity) を**前提として乗る** follow-up plan を本 plan とは別 file に切り出す。Phase 番号は本 plan の体系を流用し、依存 phase は各 plan が宣言する。

| Plan | 依存 phase (本 plan) | 目的 | 非目的 |
|---|---|---|---|
| [client-credential-injection.md](./client-credential-injection.md) | Phase 4 (E2E Noise) | credential の出所を browser に置き、host channel 経由で host credproxy に直接 push する (multi-host 環境で「host ごとに login 作業」を 1 回に集約)。**Web で完結、native client 不要**。 | gateway 経由配布 (no-domain 違反)、cross-device 同期、既存 host-sourced credential の廃止 |
| [client-as-host.md](./client-as-host.md) | Phase 3 (LAN 直結) | client 機自身で server binary を 1 つ起動し、browser が `localhost` を 1 host として扱う ("PC 1 台完結" / gateway 不在運用 / hostexec on local device)。**既存 host モデル再利用で native 化不要**。 | browser 単独での pty 実行 (原理的に不可)、PWA/Tauri 化、mobile/tablet 等 server を起動できない device 上での local hostexec |

両 plan とも本 plan の host channel / nickname store / identity / pairing を**そのまま再利用**するため、本 plan が Phase 2 まで進めば wire 変更面積は小さい (host channel に frame を 2〜3 種足すのみ)。詳細は各 plan を参照。

**設計上の制約 (両 follow-up に共通)**:
- gateway を経由しない (no-domain 原則 [[feedback-gateway-no-domain]] を踏襲)。
- Web client は **browser そのもの**として残す (native client / PWA / Tauri は導入しない)。hostexec はあくまで「client 機上で `server` を 1 つ走らせる」モデル。
- 既存の host 概念を**拡張しない**: 「local host (= client 機の `server`)」も「remote host」も同じ `Host` 型として扱い、wire / state / UI に区別を作らない (UX 上の section header だけ "This device" を default にする等の表示差に留める)。
