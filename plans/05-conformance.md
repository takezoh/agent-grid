...
`docs/orchestrator/symphony-conformance.md` として SPEC リリース時に同等の表を維持する想定。

## 厳守する項目

| SPEC §  | 項目 | 我々の対応 |
|---|---|---|
| §1 §2.2 §11.5 | Symphony は scheduler/runner であり、チケット状態遷移 (Planning → Done) を orchestrator に持たない | loki の Linear status state machine は採用しない。phase は agent prompt + agent tool で実装 |
...
逸脱は `docs/orchestrator/symphony-conformance.md` の "Documented Posture" セクションで宣言する想定。
...
| §10.5 (`linear_graphql`) | OPTIONAL extension | **`platform/mcpproxy/` 経由で実装** | Linear MCP server を agent tool として注入。SPEC が要請する input/output 形式は MCP server 側で確保 |
...
- `codex.command: claude-app-server` の場合: claude には approval 概念がないため shim が即実行。container 境界が安全保証
...
`docs/orchestrator/symphony-conformance.md` に SPEC § と test name の対応表を維持する。
