...
| §10.5 (`linear_graphql`) | OPTIONAL extension | **codex native `item/tool/call` で実装**（MCP 不採用）。`thread/start` の `dynamicTools` で `DynamicToolSpec` を advertise | SPEC は「orchestrator 自身が tool を実行」と規定し MCP は SPEC 非語彙。詳細は [issues/024](../issues/.archive/024-p8b-linear-graphql-tool.md) |
