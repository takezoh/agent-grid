...
| `assistant: tool_use` | `item/started` (dynamicToolCall) — 全ツールを一律 **`dynamicToolCall`** として emit。Bash/Edit 等を commandExecution/fileChange に推測分岐しない（固有必須フィールドの fabrication 回避）|
| `user: tool_result` | `item/completed` (dynamicToolCall) — tool_use_id で id 相関。IsError で status = completed/failed |
| `result: success` | `thread/tokenUsage/updated`(先行 emit)+ `turn_completed` + usage (per-turn を thread 累積 absolute に積み上げ) |
...
- **advertise**: `thread/start` の `dynamicTools` に `DynamicToolSpec`（`name`/`description`/`inputSchema`）を載せて宣言（`orchestrator/agent/dynamictools.go`）。Linear client 設定時のみ advertise し、agent は discover した `linear_graphql` を `item/tool/call` で発火する。詳細は [issues/024](../issues/.archive/024-p8b-linear-graphql-tool.md)。
