...
- [ ] 起動時に `tracker.TerminalIssues` で terminal state の issue を取得
- [ ] 各 identifier の workspace ディレクトリを `Remove`
- [ ] **terminal-issues fetch 失敗時は warn ログを出して起動継続** (§8.6)

### D. tick への組み込み

- [ ] reconcile を tick の **最初** に実行する関数を 012 の tick に注入 (§8.1 step 1: dispatch より前)

### E. テスト (§17.4)

- [ ] stall: `last_codex_timestamp`/`started_at` 基準の elapsed 判定、超過で kill+retry、`stall_timeout_ms<=0` で skip
- [ ] refresh: terminal→kill+clean / active→snapshot 更新 / 中間→kill のみ (workspace 残存)
- [ ] refresh 失敗時に worker 継続
- [ ] startup cleanup が terminal workspace を削除、fetch 失敗で起動継続
