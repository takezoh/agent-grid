...
3. `EvPaneSpawned` の reducer は session 不在で no-op。よってこれらは二度と解放
...
> **2026-06-27 解消** (commit 046e5558): `handleSpawnComplete` 冒頭で
> `spawnTargetAlive(SessionID, FrameID)` を呼び、reducer 状態に存在しなければ
> `discardSpawnResult` に分岐して `KillPaneWindow` (loop 同期) +
> `ReleaseFrame` / cleanup / Reaper.Remove (別 goroutine) を実行する。
> tests: `TestHandleSpawnComplete_discardsWhenFrameKilledMidSpawn` /
> `TestHandleSpawnComplete_discardsContainerFrame`.
...
- `src/client/runtime/interpret_spawn.go`（`spawnPaneWindow`, `handleSpawnComplete`,
  `discardSpawnResult`, `reapSubsystemIfLast`）
- `src/client/runtime/interpret.go`（`executeKillSessionWindow`）
