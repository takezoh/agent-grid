...
- [ ] `src/orchestrator/workflowfile/` を新設 (`package workflowfile`)
- [ ] 返却型を定義:
...
- [ ] `Load(path string) (Workflow, error)`:
  - [ ] 先頭が `---` なら次の `---` までを YAML front matter として decode
  - [ ] 残りを prompt body とし、trim して `PromptTemplate` に格納
  - [ ] front matter が無ければ全体を body 扱い、`Config` は空 map
  - [ ] YAML front matter が map/object に decode できない場合はエラー
- [ ] path 解決の precedence は呼び出し側 (007/cmd) の責務。本パッケージは受け取った path を読むだけ

### C. typed error (SPEC §5.5)

- [ ] error class を sentinel/typed error として公開:
  - [ ] `missing_workflow_file` (read 失敗)
  - [ ] `workflow_parse_error` (YAML decode 失敗)
  - [ ] `workflow_front_matter_not_a_map` (front matter が非 map)
- [ ] `errors.Is` で判別可能にする (例: `var ErrMissingWorkflowFile = errors.New(...)`)

### D. ライブラリ選定

- [ ] YAML パーサは **`gopkg.in/yaml.v3`** を採用 (go.mod に既存 / AGENTS.md「既存依存を優先」)
  - 代替: `go.yaml.in/yaml/v3` (fork, 既存だが indirect)、`goccy/go-yaml` (高速だが新規依存)。indirect → direct への昇格のみで済む yaml.v3 を選択
- [ ] front matter は `map[string]any` に decode (typed 化は 006 の責務)
