...
- [ ] tool 定義を codex `initialize` の dynamic tool list に載せる（`DynamicToolSpec` の experimental 経路）— codex 実機での動作確認が必要（schema 上 orphan のため実装保留）
...
- agent が `item/tool/call` で `linear_graphql` を呼び出すと Linear GraphQL に転送される ✅
