...
- [ ] claim state を定義: `Unclaimed` / `Claimed` / `Running` / `RetryQueued` / `Released`
- [ ] in-memory state (§16.4 の構造に対応):
  - [ ] `running` map (issue_id → run state: worker handle, identifier, issue snapshot, session_id, codex_app_server_pid, last_codex_{message,event,timestamp}, started_at, retry_attempt)
  - [ ] `claimed` set
  - [ ] `retry_attempts` map (issue_id → {attempt, identifier, error, due_at_ms, timer handle})
- [ ] token/runtime 集計フィールドは **構造だけ用意** (実集計は P6)

### B. 遷移関数 (§7.3) — I/O を持たない純粋関数中心

- [ ] `Dispatch(issue, attempt)` — running 追加 + claimed 追加 + retry_attempts 削除 (§16.4)
- [ ] `WorkerExitNormal(issue_id)` — running 削除 + continuation retry (attempt 1) スケジュール (§7.3)
- [ ] `WorkerExitAbnormal(issue_id, err, attempt)` — running 削除 + exponential backoff retry スケジュール
- [ ] `ReleaseClaim(issue_id)` — claimed/running/retry から除去 → Unclaimed
- [ ] `Snapshot()` — observability/HTTP (P7) 向けの読み取り専用 view

### C. single-authority 直列化 (§7.4)

- [ ] state mutation を単一 goroutine / mutex で直列化し duplicate dispatch を防ぐ
- [ ] worker からの結果 (codex update / exit) は channel 経由で authority に集約

### D. run attempt lifecycle (§7.2)

- [ ] 11 phase (`PreparingWorkspace`..`CanceledByReconciliation`) を enum で定義 (実遷移の駆動は 013/014、ここでは型と記録のみ)

### E. テスト (§17.4 の state 部分)

- [ ] Dispatch → running/claimed に入る、retry_attempts から消える
- [ ] WorkerExitNormal → running から消え continuation retry(attempt 1) が立つ
- [ ] WorkerExitAbnormal → backoff retry が立つ
- [ ] ReleaseClaim → 全 map から消える
- [ ] 同一 issue の重複 Dispatch を防ぐ (claimed/running チェック)
