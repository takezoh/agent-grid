...
- [ ] `orchestrator/prompt/` 新設: WORKFLOW.md prompt body をテンプレートとして `issue` / `attempt` 変数で render
- [ ] **strict 失敗**: 未知変数・未知 filter は render エラー (§5.4) → `template_render_error`
- [ ] body 空なら最小デフォルト prompt (`You are working on an issue from Linear.`) を許容 (§5.4)
- [ ] **ライブラリ選定 (validate 必須)**: Liquid 互換が要件。候補と strict 失敗対応の可否を検証してから採用:
  ...
  - → [[feedback_library_validation]] に従い、strict 失敗要件を満たす最小の選択を PR で正当化する
...
- [ ] `orchestrator/agent/` 新設、`RunAttempt(ctx, issue, attempt, emit func(Event))`:
  ...
  6. turn 完了 / 失敗 / timeout (`codex.turn_timeout_ms`) / subprocess exit を判定 (§10.3)
  7. session stop → `after_run` hook (best-effort) → 正常 exit
- [ ] §9.5 Inv1: 起動前に `cwd == workspace_path` を `workspace.VerifyCWD` で検証

### C. session 識別子 (§10.2)

- [ ] `thread_id` / `turn_id` を codex 応答から抽出し `session_id = "<thread_id>-<turn_id>"` を emit
- [ ] issue メタ (`<identifier>: <title>`) を turn/session title に載せる (protocol が許せば)

### D. 最小 emit event (§10.4)

- [ ] `session_started` / `turn_completed` / `turn_failed` を emit (timestamp 付き)
- [ ] 残りの event 種別 (`turn_input_required` 等) と token usage は **P6**

### E. テスト (§17.5)

- [ ] prompt render が `issue`/`attempt` を埋め、未知変数で失敗する
- [ ] fake codex app-server (stdio で codexclient.Server を喋る test double) で 1 turn を回し、session_started/turn_completed が emit される
- [ ] turn 失敗 → turn_failed emit + after_run best-effort 実行
- [ ] workspace/ before_run 失敗で fail する
