...
func TestResolveSocketPathFromEnv(t *testing.T) {
	t.Setenv("AG_SOCKET", "/tmp/custom.sock")
...
// TestResolveSocketPath_DataDirOverride asserts that when AG_SOCKET is
...
// TestResolveSocketPath_RoostSocketWins asserts that AG_SOCKET still wins
// over an explicit dataDirOverride. Container sandboxes set AG_SOCKET to
...
func TestResolveSocketPath_RoostSocketWins(t *testing.T) {
	t.Setenv("AG_SOCKET", "/env/wins.sock")
	got, err := ResolveSocketPath("/foo")
	if err != nil {
		t.Fatal(err)
	}
	if got != "/env/wins.sock" {
		t.Errorf("got %q, want /env/wins.sock (AG_SOCKET must beat override)", got)
	}
}
