...
type Spec struct {
	...
	// consumer side (host/event.Run dispatch) keys on this exact value —
	...
	// (src/host/driver/<agent>_event.go) — adding an event upstream
...
// Event list mirrors the consumer in src/host/driver/claude_event.go.
var Claude = Spec{
	Name: "claude",
	Events: []string{
		"SessionStart",
