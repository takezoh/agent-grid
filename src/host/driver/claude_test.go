...
import (
	...
	"github.com/takezoh/agent-grid/host/lib/claude/transcript"
...
func TestClaudePrepareLaunchWorktreeStripsFlag(t *testing.T) {
	d := NewClaudeDriver(testHome, testEventLogDir, ClaudeOptions{}, "less")
	tests := []struct {
		name    string
		command string
		...
		{"bare flag", "claude --worktree", state.LaunchOptions{}},
		{"flag with name", "claude --worktree my-branch", state.LaunchOptions{}},
		{"flag equals form", "claude --worktree=my-branch", state.LaunchOptions{}},
		{"from options", "claude", state.LaunchOptions{Worktree: state.WorktreeOption{Enabled: true}}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cs := ClaudeState{ClaudeSessionID: "uuid-W"}
			plan, err := d.PrepareLaunch(cs, state.LaunchModeCreate, "/repo", tt.command, tt.options, false)
			if err != nil {
				t.Fatalf("PrepareLaunch error: %v", err)
			}
			if plan.Command != "claude" {
				t.Errorf("Command = %q, want worktree flag stripped", plan.Command)
			}
			if !plan.Options.Worktree.Enabled {
				t.Error("expected Options.Worktree.Enabled = true")
			}
		})
	}
}
...
func TestClaudePrepareCreateWithWorktree(t *testing.T) {
	d, cs, _ := newClaude(t)
	_, launch, err := d.PrepareCreate(cs, "sess-1", "/repo", "claude --worktree", state.LaunchOptions{})
	if err != nil {
		t.Fatalf("PrepareCreate error: %v", err)
	}
	if !launch.Options.Worktree.Enabled {
	...
	if launch.Command != "claude" {
		t.Fatalf("launch command = %q, want claude", launch.Command)
	}
}
