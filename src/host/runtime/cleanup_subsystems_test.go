...
func TestCollectTrackedWorktreesCodex(t *testing.T) {
	...
	worktreePath := "/repo/.agent-grid/worktrees/alpha-beta"
...
func TestCollectTrackedWorktreesMultipleFrames(t *testing.T) {
	...
	shellDS := shellDrv.WithStartDir(shellDrv.NewState(time.Now()), "/repo/.agent-grid/worktrees/shell-wt")

	codexDrv := driver.NewCodexDriver("")
	codexDS := codexDrv.WithStartDir(codexDrv.NewState(time.Now()), "/repo/.agent-grid/worktrees/codex-wt")
	...
	for _, p := range []string{"/repo/.agent-grid/worktrees/shell-wt", "/repo/.agent-grid/worktrees/codex-wt"} {
