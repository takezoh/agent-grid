...
// fakeTracker implements schedulerTrackerAPI.
...
func (f *fakeTracker) RefreshStates(_ context.Context, _ []string) ([]ptrackerv.Issue, error) {
	return f.refreshIssues, f.refreshErr
}

func (f *fakeTracker) TerminalIssues(_ context.Context) ([]ptrackerv.Issue, error) {
...
func newReconcileScheduler(tr schedulerTrackerAPI, ws schedulerWorkspaceAPI, clockFn func() time.Time) *Scheduler {
	return &Scheduler{
		state:     NewState(),
		tracker:   tr,
		workspace: ws,
...
func TestReconcileStall_KillsAndEnqueuesRetry(t *testing.T) {
	w := &fakeWorker{}
	s := newReconcileScheduler(&fakeTracker{}, &fakeWorkspace{}, func() time.Time {
...
func TestReconcileRefresh_TerminalKillsAndCleansWorkspace(t *testing.T) {
	...
	tr := &fakeTracker{refreshIssues: []ptrackerv.Issue{refreshed}}
	s := newReconcileScheduler(tr, ws, time.Now)
...
func TestReconcileRefresh_ErrorSkipsAll(t *testing.T) {
	...
	tr := &fakeTracker{refreshErr: errors.New("api down")}
	s := newReconcileScheduler(tr, ws, time.Now)
	_ = s.state.Dispatch(issue, 1, LiveSession{Worker: w}, time.Now())
