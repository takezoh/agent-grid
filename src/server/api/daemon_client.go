...
var ErrDaemonUnavailable = errors.New("server/api: daemon unavailable")
...
func (d *DaemonClient) severSubscriber(sub *daemonSubscriber, kind, detail string) {
	slog.Warn("server/api: daemon fan-out severing slow subscriber",
...
func (d *DaemonClient) tryDialOnce(attempt int) {
	...
	d.health.Store(true)
	slog.Info("server/api: daemon connected", "sock", d.sockPath, "attempt", attempt)
}
...
func (d *DaemonClient) reconnectLoop(attempt int) int {
	...
	for {
		...
		slog.Warn("server/api: backing off before next dial",
			"sock", d.sockPath,
			"attempt", attempt,
			"delay", jitter)
		...
		slog.Warn("server/api: daemon dial failed",
			"sock", d.sockPath,
			"attempt", attempt,
			"err", d.LastError(),
			"delay", jitter)
...
func (d *DaemonClient) markDown(reason string) {
	...
	d.health.Store(false)
	d.closeAllSubs()
	slog.Warn("server/api: daemon disconnected", "reason", reason)
}
