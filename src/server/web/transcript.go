...
func handleGetTranscript(d *DaemonClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		serveSessionFile(d, w, r, "transcript")
	}
}
...
func handleGetEventLog(d *DaemonClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		serveSessionFile(d, w, r, "event-log")
	}
}

// serveSessionFile resolves the file path for the given session and kind,
// then streams from offset to EOF with ETag support.
func serveSessionFile(d *DaemonClient, w http.ResponseWriter, r *http.Request, kindMatch string) {
	...
	// c. Resolve the file path via daemon session info.
	path, err := resolveSessionFilePath(d, r, id, kindMatch)
	if err != nil {
		// resolveSessionFilePath writes the HTTP error itself when returning nil path.
		// We use sentinel errors to communicate HTTP semantics.
...
func resolveSessionFilePath(d *DaemonClient, r *http.Request, id, kindMatch string) (string, error) {
	...
	if path == "" {
		return "", fmt.Errorf("no %s for session", kindMatch)
	}
	return path, nil
}
