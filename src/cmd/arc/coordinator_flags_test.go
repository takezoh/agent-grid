...
func TestIsCoordinatorFlag(t *testing.T) {
	cases := []struct {
		arg  string
		want bool
	}{
		...
		{"-data-dir=/tmp/x", true},
		{"--data-dir=/tmp/x", true},
		{"-data-dir/x", false}, // not a flag form
		{"-other", false},
		{"--tui", false},
		{"help", false},
		{"", false},
	...
	for _, tc := range cases {
		if got := isCoordinatorFlag(tc.arg); got != tc.want {
			t.Errorf("isCoordinatorFlag(%q) = %v, want %v", tc.arg, got, tc.want)
		}
	}
}

func TestClassifyCommandRoutesDataDirFlagToRoost(t *testing.T) {
	cases := [][]string{
		{"-data-dir", "/tmp/x"},
		{"--data-dir", "/tmp/x"},
		{"-data-dir=/tmp/x"},
	...
	for _, args := range cases {
		if got := classifyCommand(args); got != commandKindRoost {
			t.Errorf("classifyCommand(%v) = %v, want commandKindRoost", args, got)
		}
	}
}
...
func TestParseDaemonArgs(t *testing.T) {
	cases := []struct {
		...
		{name: "space form", args: []string{"-data-dir", "/var/lib/x"}, want: "/var/lib/x"},
		{name: "equals form", args: []string{"-data-dir=/var/lib/x"}, want: "/var/lib/x"},
		{name: "double dash", args: []string{"--data-dir", "/var/lib/x"}, want: "/var/lib/x"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got != tc.want {
				t.Errorf("data-dir = %q, want %q", got, tc.want)
			}
		})
	}
}
...
// config BEFORE logger init: the captured DataDir at the moment runCoordinator
...
func TestRunMainInjectsDataDirIntoConfig(t *testing.T) {
	...
	// We can't reach the cfg from runCoordinator's stub directly; instead we
	// stub initLoggerWithDataDir to capture the dir runMain hands it.
...
// ROOST_DATA_DIR so a downstream fresh loadConfig() (runCoordinator does this)
...
func TestRunMainExportsDataDirToEnv(t *testing.T) {
	...
	if got := os.Getenv("ROOST_DATA_DIR"); got != flagDir {
		t.Errorf("ROOST_DATA_DIR = %q, want %q (flag must be exported so a "+
			"fresh ResolveDataDir() in runCoordinator returns the same path)",
			got, flagDir)
	}
}
...
func TestRunMainBadDaemonFlagExitsTwo(t *testing.T) {
	...
	parseDaemonArgsFn = func(args []string) (string, error) {
		return "", errors.New("bad flag")
	}
	called := false
	runCoordinatorFn = func() error { called = true; return nil }
	...
	if called {
		t.Fatal("runCoordinator must not be called when daemon flag parse fails")
	}
