...
func handleMCPToolCall(client *proto.Client, token string, msg mcpEnvelope) *mcpEnvelope {
	...
	default:
		return &mcpEnvelope{JSONRPC: "2.0", ID: msg.ID, Error: &mcpRPCError{Code: -32601, Message: "unknown tool"}}
	}
	if err != nil {
		return mcpToolError(msg.ID, err)
	}
	return &mcpEnvelope{
		JSONRPC: "2.0",
		ID:      msg.ID,
		Result:  mcpStructuredResult(resp),
	}
}

func mcpStructuredResult(v any) mcpToolResult {
...
func mcpToolError(id json.RawMessage, err error) *mcpEnvelope {
