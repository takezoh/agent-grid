...
// The type comparison is case-insensitive and trims surrounding whitespace to
// match the Elixir reference: String.downcase(String.trim(relation_type)) == "blocks".
func normalizeBlockers(nodes []rawRelNode) []tracker.Blocker {
	var out []tracker.Blocker
	for _, n := range nodes {
		if strings.ToLower(strings.TrimSpace(n.Type)) != "blocks" {
			continue
		}
