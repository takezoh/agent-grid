...
func TestResolveDaemonSocket_FlagPriority(t *testing.T) {
	...
	got := socketpath.ResolveDaemonSocket("/explicit/sock", "ARC_SOCK", "arc.sock")
...
func TestResolveDaemonSocket_EnvFallback(t *testing.T) {
	t.Setenv("ARC_SOCK", "/from/env")
	got := socketpath.ResolveDaemonSocket("", "ARC_SOCK", "arc.sock")
	if got != "/from/env" {
...
func TestResolveDaemonSocket_HomeFallback(t *testing.T) {
	...
	got := socketpath.ResolveDaemonSocket("", "ARC_SOCK", "arc.sock")
	want := filepath.Join(tmp, ".agent-reactor", "arc.sock")
	if got != want {
...
func TestResolveDaemonSocket_TmpFallback(t *testing.T) {
	...
	t.Setenv("ARC_SOCK", "")
	got := socketpath.ResolveDaemonSocket("", "ARC_SOCK", "arc.sock")
	want := filepath.Join(os.TempDir(), "arc.sock")
	if got != want {
		t.Errorf("expected %s, got %s", want, got)
	}
}
