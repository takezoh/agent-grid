...
| **Subsystem** | Runtime-owned execution backend. `cli` launches a single command in a single pane; `stream` fronts long-lived structured backends such as Codex App Server | Usually one tmux **pane** per subsystem |
...
| Resident tracking | `SubsystemID -> PaneID`, `FrameID -> TargetID` | Visible panes belong to subsystems; frames track logical execution targets. CLI uses one frame per subsystem, while Codex maps many frames (threads) onto one stream subsystem pane |
...
| Structured stream integration | `codex app-server` → bridge/gateway → `proto.CmdSubsystemEvent` → `EvSubsystem` → `reduceSubsystem` → `Driver.Step(DEvSubsystem)` | Used by Codex. The daemon starts one `codex app-server --listen unix://<runDir>/codex.sock` per project (`driver.CodexAppServerSockName`) and connects via **WebSocket-over-UDS** (`runtime/codex_backend.go`). `sockbridge` bridges TCP loopback port `driver.CodexAppServerLoopbackPort` (8282) to the UDS for the TUI's `--remote ws://` URL. `LaunchSubsystem` (`cli` / `stream`) distinguishes CLI-spawned agents from stream-backed ones; `runtime/subsystem.go` dispatches accordingly. The stream layer emits structured tool / approval / plan / diff / message / thread lifecycle events, and `TargetID` carries the logical thread identity |
