...
- **Architecture layer** — which part of the system: `platform/` (shared infrastructure), `client/` (the client TUI), or `orchestrator/` (the autonomous Symphony pipeline).
...
| Audience \ Layer | platform | client (arc) | orchestrator | Cross-cutting |
|---|---|---|---|---|
| **User** | [sandbox setup](user/sandbox.md) | [client TUI](user/reactor-tui.md) | [orchestrator](user/orchestrator.md) | [getting started](user/getting-started.md) |
...
- [client TUI](user/reactor-tui.md) — key bindings, session states, command palette, `~/.agent-reactor/settings.toml`
...
- [client/](technical/client/README.md) — agent-reactor's client: Functional Core / Imperative Shell, the state machine, drivers, subsystems, IPC
