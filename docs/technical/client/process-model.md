...
Every Codex frame flows through **both**: `stream.Backend.BindFrame` registers a per-frame binding (empty threadID for fresh cold-start, pre-bound for recovery), rewrites `Plan.Command` to `codex --remote` / `codex resume <id> --remote`, then `PtyBackend.SpawnFrame` runs that command in a pty. Backend itself never issues `thread/start` or `thread/resume` — the codex CLI owns the thread lifecycle, and backend adopts the resulting broadcast via `handleThreadStarted` under the `initSem` serialisation invariant ([ADR-0081](../../adr/0081-codex-frame-init-serialize.md)).
