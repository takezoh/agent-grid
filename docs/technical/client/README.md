...
| Hook event target identification | Inject a frame-scoped env var at frame-spawn time | Env vars are race-free at kernel exec level. See [state monitoring](state-monitoring.md#hook-event-routing-and-race-free-identification). |
...
- [Process model](process-model.md) — daemon process, pty frame model, rendering responsibilities
