---
change: change-20260626-2026-06-26-terminal-scrollback
role: requirements
---
...
- **FR-005 — configuration plumbing**
  `~/.agent-grid/settings.toml` の `[terminal] scrollback_lines = N` で daemon が起動する全 session の scrollback 上限を上書きできる。default は 10,000。`[terminal]` セクション省略時は default が適用される。
