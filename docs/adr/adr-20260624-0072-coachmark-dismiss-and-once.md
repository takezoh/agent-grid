...
Related: [ADR 0064](../adr/adr-20260624-0064-reduced-motion-single-guard.md), [ADR 0070](../adr/adr-20260624-0070-fontsize-persist-clamp.md)
...
(3) Coachmark の DOM は通常 `<div role='status'>` (`aria-live` は使わず popup として SR にも自然に読まれる) — `aria-live` は `AriaLiveStatus` single slot 専用 ([ADR 0073](../adr/adr-20260624-0073-arialive-debounce-and-jump-fab-seed-stability.md))。
...
(5) fade-out 250ms は [ADR 0064](../adr/adr-20260624-0064-reduced-motion-single-guard.md) の view.css 末尾 `@media (prefers-reduced-motion: reduce)` single guard block に追記し reduce では即時化。

(6) `hintSeen` の永続化は [ADR 0070](../adr/adr-20260624-0070-fontsize-persist-clamp.md) の `createPersistedValue<boolean>(key, ...)` adapter を共有する。
