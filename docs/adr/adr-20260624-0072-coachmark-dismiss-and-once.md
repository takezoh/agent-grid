---
id: adr-20260624-0072-coachmark-dismiss-and-once
kind: adr
title: ADR 0072 — Coachmark dismiss は tap or 5s の早い方とし `hintSeen` 書込は初回 render 時に冪等化する
status: accepted
created: '2026-06-24'
updated: '2026-07-04'
tags:
...
Related: [ADR 0064](adr-20260624-0064-reduced-motion-single-guard.md), [ADR 0070](adr-20260624-0070-fontsize-persist-clamp.md)
...
Related spec: [Web Terminal Mobile UX spec.md](../changes/change-20260626-web-terminal-mobile-ux/requirements.md) — `FR-MOB-COACH-001/002`
...
(3) Coachmark の DOM は通常 `<div role='status'>` (`aria-live` は使わず popup として SR にも自然に読まれる) — `aria-live` は `AriaLiveStatus` single slot 専用 ([ADR 0073](adr-20260624-0073-arialive-debounce-and-jump-fab-seed-stability.md))。
...
(5) fade-out 250ms は [ADR 0064](adr-20260624-0064-reduced-motion-single-guard.md) の view.css 末尾 `@media (prefers-reduced-motion: reduce)` single guard block に追記し reduce では即時化。

(6) `hintSeen` の永続化は [ADR 0070](adr-20260624-0070-fontsize-persist-clamp.md) の `createPersistedValue<boolean>(key, ...)` adapter を共有する。
