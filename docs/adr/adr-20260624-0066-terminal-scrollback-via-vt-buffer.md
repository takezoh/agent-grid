...
(4) 設定は `~/.agent-grid/settings.toml` の `[terminal] scrollback_lines = N` として `MonitorConfig` パターンに揃える。default は `10000` (xvt のデフォルトと一致)。`cmd/server/coordinator.go` の `buildRuntime()` で `runtime.NewPtyBackend(cfg.Terminal.ScrollbackLines)` として渡し、`PtyBackend` が `SpawnFrame` / `RespawnFrame` の構築する `termvt.Spec` に伝搬する。
