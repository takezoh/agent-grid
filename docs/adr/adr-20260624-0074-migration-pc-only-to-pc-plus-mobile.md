---
id: adr-20260624-0074-migration-pc-only-to-pc-plus-mobile
kind: adr
title: ADR 0074 — PC 専用 `TerminalPane.tsx` を PC + モバイル両対応へ refactor する移行戦略 (gate 内に閉じ込め + PC baseline test で機構的 rollback)
status: accepted
created: '2026-06-24'
updated: '2026-07-04'
tags:
...
`ux.md` `reference_ux`『専用モバイルビュー: rejected』に違反 / subscribe 所有権 ([ADR 0030](adr-20260624-0030-terminal-keyed-remount.md) keyed remount) が壊れる / コードベース二重化。**却下**。
...
- 別 `TerminalPaneMobile.tsx` を新設しないため subscribe 所有権 ([ADR 0030](adr-20260624-0030-terminal-keyed-remount.md) keyed remount) と整合
