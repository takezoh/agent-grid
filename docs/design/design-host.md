...
User-facing `settings.toml` fields and defaults live in the source code (`host/config/config.go`).

## File Structure

Source tree is split into three top-level trees under `src/` — see ARCHITECTURE.md "Layer Structure".
`src/platform/` holds shared infrastructure, `src/host/` holds client-specific code, `src/cmd/` holds binary entry points.
...
├── cmd/bridge/  Bridge binary (thin container-side client; uses host/proto)
...
├── host/state/        Pure domain layer (no I/O, no goroutine)
...
├── host/driver/       Driver implementations — value-type plugins (no goroutines, no I/O)
...
├── host/runtime/      Imperative shell — event loop + Effect interpreter
...
├── host/proto/        Typed IPC wire layer — imports state/view only
│                        (safe for bridge)
...
├── host/proto/sessions/    Session management layer — imports proto + state
│                             (NOT used by bridge)
...
├── host/lib/          Client-only helpers (not used by bridge)
...
|---|---|
| `host/state/` | Pure domain layer — `State`, `Event`, `Effect`, `Reduce`. No I/O, no goroutines. Imports only stdlib + stdlib-only internal packages (`features`). |
| `host/state/view/` | Wire-safe view types — `Status`, `View`, `Card`, `Tag`. Stdlib-only; no `state` import. |
| `host/driver/` | Driver implementations — value-type plugins + per-frame `DriverState`. No I/O. |
| `host/runtime/` | Imperative shell — single event loop, Effect interpreter, backend abstraction. |
| `host/runtime/worker/` | Worker pool — slow I/O jobs (summarize, transcript parse, git, github fetch). |
| `host/runtime/subsystem/` | `Subsystem`/`Factory` interfaces + the `cli` and `stream` implementations. The only place in `runtime/` allowed to import `driver/<tool>`. |
| `host/proto/` | Typed IPC wire layer — Command / Response / ServerEvent sum types + codec. Imports `state/view` only. |
| `host/proto/sessions/` | Session-management helpers wrapping `proto.Client`. Imports `state`. |
| `host/tools/` | Operator tool abstraction (palette-style tool invocation surfaced through IPC). |
| `clients/ui/` (repo root) | Browser frontend assets (React + xterm.js), served by `cmd/uihost` via `src/uihost`. |
| `host/config/` | TOML loading, DataDir injection, SandboxResolver. |
| `client/cli/` | Subcommand registry — tool-specific subcommands registered via `init()`. |
| `host/lib/peers/` | Peers MCP server (IPC specific to the client). |
| `host/lib/{claude,codex}/transcript/` | Transcript renderers (depend on `state` for frontend integration). |
...
- src/cmd/server/
- clients/ui/src/
- src/host/lib/agenthook/
- src/server/api/
- src/host/runtime/subsystem/stream/
- src/host/runtime/pty_backend.go
...
|---|---|---|---|---|
| `stream.Backend` (`host/runtime/subsystem/stream/`) | session (`SubsystemID` keyed on session) | `Subsystem` interface | `codex app-server --listen unix://...` via `agentlaunch.Spawn` (argv-direct) | no — plain stdio pipes, `procgroup` |
| `PtyBackend` (`host/runtime/pty_backend.go`) | frame (1 per FrameID) | `FrameBackend` interface | rewritten `Plan.Command` = `codex --remote unix://...` via `termvt.Manager` | yes — `/dev/ptmx` master owned by daemon |
...
<!-- migrated_from: docs/technical/host/state-monitoring.md -->
...
AG_E2E_APPSERVER_ARGS="--flag value" \
  go test -tags e2e -run TestStreamRoutingE2E ./host/runtime/subsystem/stream/ -v
...
cd src && TMPDIR=/tmp go test ./host/runtime/subsystem/stream/

# concurrency check
cd src && go test -race ./host/runtime/subsystem/stream/

# active fuzzing
cd src && go test -run x -fuzz 'FuzzStreamRouting$' -fuzztime=30s \
  ./host/runtime/subsystem/stream/

# fidelity backstop against a real app-server (opt-in; see stream-backend-e2e.md)
AG_E2E_CODEX_BIN=$(which codex) \
  go test -tags e2e -run TestStreamRoutingE2E ./host/runtime/subsystem/stream/
```
