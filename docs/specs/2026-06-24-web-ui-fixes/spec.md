...
| Q2 | `RunStateBadge` の active 定義 (`running` のみ vs `running + waiting`) | **`running + waiting` を active とする** | [ADR 0032](../../adr/0032-runstate-spinner-additive.md) Decision。能動待機表現と一貫 |
...
| Q5 | 起動直後に title/subtitle 未確定で生 id が一瞬見える | **現挙動を維持** | プレースホルダ化はせず ([ADR 0033](../../adr/0033-display-label-empty-policy.md) Consequences) |
