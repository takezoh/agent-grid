...
- **FR-002 — scrollback frame elision when empty**
  scrollback バッファが空 (fresh session 直後 / 全画面 TUI が alt-screen を専有している間) のとき、`Subscribe()` の seed は visible grid 1 frame のみで構成され、scrollback frame は emit されない。
...
- **FR-007 — alt-screen does not falsify history**
  alt-screen TUI (vim / less / htop) の使用中、その描画は primary screen の scrollback に積まれない。alt-screen 終了 (DECSET 1049 reset) 後の subscribe は alt-screen 開始前の primary screen 履歴を引き継ぐ。これは VT spec 通りであり、本 feature 由来の new behaviour ではないが、テストで明示的に pinning する。
...
- ❌ raw PTY バイトを replay する設計に戻すと、resize / alt-screen 遷移後の絶対座標 ANSI が新 cols×rows に当たらず全画面 TUI が崩壊する (ADR-0066 で却下した raw / asciicast 方式の根拠)。
